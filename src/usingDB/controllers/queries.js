...
//const PQ_getAllUsers = new PQ('SELECT user_info.*, authentication.token FROM user_info INNER JOIN authentication ON user_info.user_id = authentication.user_id');
...
const PQ_userLogin = new PQ('SELECT email, user_password, user_id FROM user_info WHERE email = $1');
...
const PQ_getUserToken = new PQ('SELECT token FROM authentication WHERE user_id = $1');
